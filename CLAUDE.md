...
- On a terminal too narrow for the columns' floor plus the tail, line 2 drops rate limits and then the cache icon. Line 1's tail is not optional, so a long branch with large diff stats still overflows on a narrow terminal. The floor is `line1Outside + COLS_FLOOR`, so it moves with the digits in the tail: 31 + 30 = 61 for `↑12↓34 +1234/-5678` (`MIN_SUPPORTED_COLS` in the tests), 65 for `↑123↓456 +12345/-67890`
