...
- Nerd Font icons live in the private use area, which Unicode calls Ambiguous, so the terminal's width table decides how far the cursor moves. The font only decides whether the glyph fits that space: Bizin Gothic NF drew a two-cell glyph into a one-cell advance, which is what made the columns look cramped, and Cica draws to match a two-cell advance. `WIDE_RANGES` fixes the arithmetic at two, so the terminal has to agree (WezTerm `cell_widths`, nightly builds only; Windows Terminal only has the broader `compatibility.ambiguousWidth`; Ghostty has no way to do it, because its width table is Unicode's own and the private use areas are Ambiguous there). README's Requirements carries the per-terminal detail and a one-line check. The layout reads the two through `visualWidth(ICON_FOLDER)` instead of writing it in each place — but nothing detects the terminal, so a one-cell terminal needs the ranges removed
