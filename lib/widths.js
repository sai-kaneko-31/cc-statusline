...
// table — that table is what advances the cursor, not the font's metrics.
// Two is the target, and Cica draws its icons that wide so the glyph fills the
// space instead of spilling out of it. Set STATUSLINE_ICON_CELLS=1 for a
// terminal that advances one and cannot be told otherwise. README's
...
const WIDE_RANGES = ICON_CELLS === 2
  ...
  : ALL_WIDE_RANGES.filter(
    ([lo]) => !PRIVATE_USE_RANGES.some(([puLo]) => puLo === lo)
  );
